 ...
 *
 *      It should be slightly more tolerant of other devices active
...
static void raw(int fd)
{
    ...
    if (cfsetospeed(&it, B19200) < 0)
    {
        err("set o speed");
    }

    if (cfsetispeed(&it, B19200) < 0)
    {
        err("set i speed");
    }

    if (tcsetattr(fd, TCSANOW, &it) < 0)
    {
        err("set attr");
    }
}
...
void senddirdata(int disk, int sec)
{
    ...
    if (sec >= 4 && sec < 4 + 64 * 5)
    {
        ...
        {
            if (disks[disk].afileno)
            {
                close(disks[disk].afileno);
            }
            disks[disk].secoff = 0;
            disks[disk].afileno = fn;
            rewinddir(disks[disk].dir);
            readdir(disks[disk].dir); // read away "." and ".." (?)
            readdir(disks[disk].dir);
            for(i = 0; i <= fn; ++i)
            {
                de = readdir(disks[disk].dir);
...
static void loaddisk(char * path, int disk)
{
    ...
    if (!disks[disk].blank || exists)
    {
...

        {
            ...
            if (((buf.st_size - ATRHEAD) % 256) == 128)
            {
                disks[disk].seekcode = atr;
            }
            if (((buf.st_size) % 128) == 0)
            {
                disks[disk].seekcode = xfd;
            }
        ...
        if (disks[disk].seekcode != xfd)
        {
            ...
            paragraphs = atr.seccountlo + atr.seccounthi * 256 + atr.hiseccountlo * 256 * 256 + atr.hiseccounthi * 256 * 256 * 256;
...
int firstgood(int disk, int sec)
{
    for(int i = TRACKSTART(sec); i < sec; ++i)
    {
        if (!disks[disk].track[TRACK18(i)].bad[OFF18(i)])
        {
            return i;
        }
    }
...
void addtiming(int disk, int sec)
{
    ...
    {
        uspr = diff; /* Observed microseconds for one revolution */
        if (!quiet)
        {
            printf(" %d.%03d RPMs ", RPM(uspr), RPM3(uspr));
        }
        goto done;
    }
    usps = uspr / 18;
    ...
    if (revs > 1)
    {
        if (!quiet)
        {
            printf(" %d revolutions (%d us) [delayed read]", revs, diff);
        }
        goto done;
    }
    ...
    if (disks[disk].lastsec != fgs)
    {
        ...
        if (disks[disk].prevsec == fgs || disks[disk].track[TRACK18(disks[disk].prevsec)].offset[OFF18(disks[disk].prevsec)])
        {
            ...
            if (!quiet)
            {
                printf(" sec %d is %d.%02d sectors after sec %d [RECORDED]",
                    disks[disk].lastsec,
...
static void decode(unsigned char * buf)
{
    ...
    if (rs >= 0)
    {
        if (!quiet)
        {
            printf("[Serial ports not supported]\n");
        }
        return;
    }

    int sec = buf[2] + 256 * buf[3];
    ...
    switch (buf[1])
    {
        case 'B':
            if (!disks[disk].active)
            {
                ...
                if (!quiet)
                {
                    printf("announce bad sector %d: ", sec);
                }
            }
        case 'R':
            if (!quiet)
        ...
        case 'S':
            ...
            ack('A');
            {
                ...
                if (disks[disk].secsize == 128 && disks[disk].seccount > 720)
                {
                    status[0] = 0x80;
                }
        ...
        case 'N':
            if (!quiet)
            {
                printf("815 configuration block read");
            }
            if (!disks[disk].active)
            {
                break;
        ...
        case 'O':
            if (!quiet)
            {
                printf("815 configuration block write (ignored)");
            }
            if (!disks[disk].active)
            {
                break;
...
{
    int r = ioctl(fd, TIOCMIWAIT, TIOCM_RNG);
}
...
int afnamecpy(char *an, const char *n)
{
    for (int i = 0; i < 11; ++i)
    {
        an[i]=' '; // Space fill the Atari name
    }
    ...
    if (*n)
    {
        return 0; /* Extension too long or more than 11 characters */
    }
